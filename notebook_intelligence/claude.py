...
# Flag to track if neurodesk RAG retriever is available
...
NEURODESK_RAG_SYSTEM_PROMPT = """You have access to the Neurodesk documentation and example notebooks search tool (neurodesk-search).

**CRITICAL: You MUST use the neurodesk-search tool BEFORE writing ANY neuroimaging code.**
...
When the user asks about neuroimaging analysis or code:
1. FIRST use neurodesk-search with their query to find relevant examples
2. Review the returned code snippets and documentation carefully
3. Use the examples as a reference when writing code for the user
4. Cite the source notebook when applicable
5. Follow the exact command syntax and parameters shown in the examples

This ensures your code follows Neurodesk best practices and uses the correct tool versions, parameters, and file formats.
"""
...
@tool("neurodesk-search", "Search Neurodesk documentation and example notebooks for neuroimaging code examples. Use this BEFORE writing any neuroimaging code.", {"query": str})
async def neurodesk_search(args) -> str:
    ...
    if not _NEURODESK_RAG_AVAILABLE:
        return tool_text_response("Neurodesk RAG retriever is not available. Please install the required dependencies: langgraph, langchain-chroma, langchain-openai, chromadb.")

    # Get API key from config or environment
    api_key = _get_neurodesk_openai_api_key()
    if not api_key:
        return tool_text_response("OpenAI API key not configured. Please set OPENAI_API_KEY environment variable or configure it in Notebook Intelligence settings under 'neurodesk_rag_settings.openai_api_key'.")

    try:
        query = args.get('query', '')
        if not query:
            return tool_text_response("Error: query parameter is required")

        chunks = retrieve(query, api_key=api_key)

        if not chunks:
            return tool_text_response(f"No relevant Neurodesk documentation found for query: '{query}'. Try rephrasing your query or using different neuroimaging terminology.")
...
"""
        return tool_text_response(response_header + "\n---\n".join(results))

    except Exception as e:
        log.error(f"Error searching Neurodesk documentation: {e}")
        return tool_text_response(f"Error searching Neurodesk documentation: {str(e)}")
...
class ClaudeCodeChatParticipant(BaseChatParticipant):
    ...
    def _create_client_options(self) -> ClaudeAgentOptions:
        ...
        # Build tools list - always include neurodesk_search if available
        nbi_tools = [create_new_notebook, add_markdown_cell, add_code_cell, get_number_of_cells, get_cell_type_and_source, get_cell_output, set_cell_type_and_source, delete_cell, insert_cell, run_cell, save_notebook, rename_notebook, run_command_in_jupyter_terminal, open_file_in_jupyter_ui]
        if _NEURODESK_RAG_AVAILABLE:
            nbi_tools.append(neurodesk_search)
        ...
        # Always enable MCP server if neurodesk RAG is available OR jupyter UI tools are enabled
        if jupyter_ui_tools_enabled or _NEURODESK_RAG_AVAILABLE:
            mcp_servers["nbi"] = self._jupyter_ui_tools_mcp_server
        allowed_tools = []
        # Always allow neurodesk_search if available
        if _NEURODESK_RAG_AVAILABLE:
            allowed_tools.append("mcp__nbi__neurodesk-search")
    ...
    def _create_system_prompt(self, jupyter_ui_tools_enabled: bool) -> str:
        neurodesk_prompt = NEURODESK_RAG_SYSTEM_PROMPT if _NEURODESK_RAG_AVAILABLE else ""
